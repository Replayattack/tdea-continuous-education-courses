...
it('prints the help message when it doesn\'t receive all command options',
  async () => {
...
it('prints the successful message when a student is enrolled', async () => {
  const { stdout: result } = await execFile(
    'node',
    ['index.js', 'enroll', '-i', 123456789, '-c', 1, '-n', 'John Doe']
  )
  expect(result.trimRight())
    .toEqual('El estudiante fue pre-matriculado correctamente.')
})
